...
//            << std::endl;
  ...
  for (int i = 0; i < windowCount; i++) {
    ...
    cv::rectangle(colorLane, tl, br, cv::Scalar(0, 0, 0), 3);
    ...
    for (int j = tlX; j <= brX; j++) {
	  ...
	  // colorLane.at<cv::Vec3b>(cv::Point(j, k)) = cv::Vec3b(0, 0, 255);
          //colorLane.at<cv::Vec3b>(cv::Point(j, k)) = cv::Vec3b(255, 255, 255);
          if (pointFlag % 10 == 0) {
...
bool LaneDetectionModule::detectLane(std::string videoName) {
  ...
  video.open("LaneDetection_output.avi", cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 20,
                        cv::Size(1280, 720));
  std::cout << "create video write" << std::endl;
  ...
  for (;;) {
    ...
    //if(frameNumber > 10 && frameNumber < 20){
    //   imwrite("result_gaussianBlurImage" + std::to_string(frameNumber) + ".jpg", gaussianBlurImage);
    //}
  // Step 4: Extract the region of interest
    ...
    // Display routine
    //if(frameNumber == 1){
    //  imwrite("result_output" + std::to_string(frameNumber) + ".jpg", finalOutput);
    //}
